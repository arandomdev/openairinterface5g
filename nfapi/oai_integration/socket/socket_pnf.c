...
int pnf_pack_and_send_p5_msg(void *pnf_ptr, nfapi_nr_p4_p5_message_header_t *msg, uint32_t msg_len)
{
  ...
  return socket_send_p5_msg(pnf->sctp, pnf->p5_sock, NULL, pnf->tx_message_buffer, packed_len, 0);
}

int send_p7_msg(pnf_p7_t* pnf_p7, uint8_t* msg, uint32_t len)
{
  ...
  if ((sendto_result = socket_send_p7_msg((int)pnf_p7->p7_sock, &remote_addr, msg, len)) < 0)
