...
static int send_p5_msg(vnf_t *vnf, nfapi_vnf_pnf_info_t *pnf, const void *msg, int len, uint8_t stream)
{
...
int vnf_pack_and_send_p5_msg(void *vnf_ptr, uint16_t p5_idx, nfapi_nr_p4_p5_message_header_t *msg, uint32_t msg_len)
{
  ...
  if (pnf) {
    ...
    if (NFAPI_MODE == NFAPI_MODE_AERIAL) {
      ...
      packedMessageLength = vnf->_public.pack_func(msg,
                                                   msg_len,
...
{
  vnf_p7_t *vnf_p7 = (vnf_p7_t *)vnf_p7_ptr;
  ...
  if (p7_connection) {
    ...
    if (len > vnf_p7->_public.segment_size) {
      ...
      for (segment = 0; segment < segment_count; ++segment) {
        ...
        nfapi_nr_p7_update_transmit_timestamp(buffer,
                                              calculate_transmit_timestamp(p7_connection->mu,
                                                                           p7_connection->sfn,
                                                                           p7_connection->slot,
                                                                           vnf_p7->slot_start_time_hr));
  ...
  if (1) {
...

      {
        ...
        if (vnf->sctp) {
          ...
          recvmsg_result = recv(pnf->p5_sock,
                                read_buffer,
                                message_size,
                                0);
        }
        ...
        if (recvmsg_result == -1) {
          ...
          // todo - handle the events
            ...
            if ((flags & 0x80) == 0x80 || !vnf->sctp) {
              ...
              int tmp = errno;
              NFAPI_TRACE(NFAPI_TRACE_WARN, "sctp_recvmsg: unhandled mode with flags 0x%x and error %s errno:%d\n", flags,strerror(tmp), tmp);
