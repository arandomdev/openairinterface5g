...
void *configure_nr_p7_vnf(void *ptr)
{
  ...
  p7_vnf->config->nr_pack_and_send_p7_msg = &wls_vnf_nr_pack_and_send_p7_message;
  ...
  p7_vnf->config->nr_pack_and_send_p7_msg = &vnf_pack_and_send_p7_msg;
  NFAPI_TRACE(NFAPI_TRACE_INFO, "[VNF] Creating VNF NFAPI P7 start thread %s\n", __FUNCTION__);
  pthread_create(&vnf_p7_start_pthread, NULL, &vnf_nr_start_p7_thread, p7_vnf->config);
#endif
  ...
  p7_vnf->config->hdr_unpack_func = &fapi_nr_p7_message_header_unpack;
  p7_vnf->config->pack_func = &fapi_nr_p7_message_pack;
  p7_vnf->config->nr_pack_and_send_p7_msg = &fapi_nr_pack_and_send_p7_message;
#endif
  return 0;
}
...
static char* error_ind_code_to_str(nfapi_nr_phy_notifications_errors_e error_code)
{
  switch (error_code) {
...
int nr_error_ind_cb(nfapi_vnf_config_t *config, int p5_idx, nfapi_nr_error_indication_scf_t *resp) {
  NFAPI_TRACE(NFAPI_TRACE_INFO, "[VNF] Received NFAPI_NR_PHY_MSG_TYPE_ERROR_INDICATION idx:%d phy_id:%d\n", p5_idx, resp->header.phy_id);
  NFAPI_TRACE(NFAPI_TRACE_INFO, "[VNF] Previous message 0x%02x resulted in an error on the PNF \n", resp->message_id);
  NFAPI_TRACE(NFAPI_TRACE_INFO, "[VNF] Received error code 0x%02x (%s)\n", resp->error_code, error_ind_code_to_str(resp->error_code));
...
void configure_nr_nfapi_vnf(eth_params_t params)
{
  ...
  config->nr_pack_and_send_p5_msg = &wls_vnf_nr_pack_and_send_p5_message;
  printf("WLS MODE PNF\n");
  NFAPI_TRACE(NFAPI_TRACE_INFO, "[PNF] Creating WLS VNF NFAPI start thread %s\n", __FUNCTION__);
  //wls_fapi_pnf_nr_start_thread(config);
  pthread_create(&vnf_p5_init_and_receive_pthread, NULL, &wls_fapi_vnf_nr_start_thread, config);
  pthread_setname_np(vnf_p5_init_and_receive_pthread, "NFAPI_WLS_VNF");
#endif
  ...
  config->nr_pack_and_send_p5_msg = &vnf_pack_and_send_p5_msg;
  ...
  config->hdr_unpack_func = &fapi_nr_message_header_unpack;
  config->pack_func = &fapi_nr_p5_message_pack;
  config->nr_pack_and_send_p5_msg = &aerial_pack_and_send_p5_msg;
  set_config(config);
  NFAPI_TRACE(NFAPI_TRACE_INFO, "[VNF] Created VNF NFAPI start thread %s\n", __FUNCTION__);
