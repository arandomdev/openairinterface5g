...
int wls_pnf_nr_pack_and_send_p5_message(void *pnf_ptr, nfapi_nr_p4_p5_message_header_t *msg, uint32_t msg_len)
{
  pnf_t *pnf = (pnf_t*)pnf_ptr;
...
int wls_pnf_nr_pack_and_send_p7_message(void* pnf_p7_ptr,nfapi_nr_p7_message_header_t *msg, uint32_t msg_len)
{
  UNUSED_VARIABLE(pnf_p7_ptr);
...
static void procPhyMessages(uint32_t msg_size, void *msg_buf, uint16_t msg_id)
{
  ...
  switch (msg_id) {
    ...
    case NFAPI_NR_PHY_MSG_TYPE_CONFIG_REQUEST:
      pnf_nr_handle_config_request(_this,msg_buf, msg_size + NFAPI_NR_P5_HEADER_LENGTH);
      break;

    case NFAPI_NR_PHY_MSG_TYPE_START_REQUEST:
      printf("\n NFAPI_NR_PHY_MSG_TYPE_START_REQUEST");
      pnf_nr_handle_start_request(_this,msg_buf, msg_size + NFAPI_NR_P5_HEADER_LENGTH);
      break;
