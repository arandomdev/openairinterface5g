...
static uint16_t set_snssai_config(nssai_t *nssai, uint8_t k, uint8_t l)
{
  ...
  for (int s = 0; s < num_ssi; ++s) {
    ...
    AssertFatal(nssai->sd <= 0xffffff, "SD cannot be bigger than 0xffffff, but is %d\n", nssai->sd);
  }
  return num_ssi;
}
...
static uint8_t set_plmn_config(plmn_id_t *p, uint8_t idx)
{
  ...
  sprintf(gnbpath, "%s.[%i]", GNB_CONFIG_STRING_GNB_LIST, idx);
                             ...
                             int max_cell_info)
{
  ...
  *info->tac = *GNBParamList.paramarray[0][GNB_TRACKING_AREA_CODE_IDX].uptr;
  ...
  // SNSSAI
  info->num_ssi = set_snssai_config(info->nssai, 0, 0);
...
void RCconfig_nr_macrlc(configmodule_interface_t *cfg)
{
  ...
  char aprefix[MAX_OPTNAME_SIZE * 2 + 8];
  sprintf(aprefix, "%s.[%d].%s", GNB_CONFIG_STRING_GNB_LIST, 0, GNB_CONFIG_STRING_TIMERS_CONFIG);
        ...
        for (j=0; j < GNBSParams[GNB_ACTIVE_GNBS_IDX].numelt; j++) {
          if (strcmp(GNBSParams[GNB_ACTIVE_GNBS_IDX].strlistptr[j], *(GNBParamList.paramarray[k][GNB_GNB_NAME_IDX].strptr)) == 0) {
            ...
            for (int l = 0; l < NGAP_REGISTER_GNB_REQ(msg_p).num_plmn; ++l) {
              ...
              // SNSSAI
              NGAP_REGISTER_GNB_REQ(msg_p).plmn[l].num_nssai = set_snssai_config(NGAP_REGISTER_GNB_REQ(msg_p).plmn[l].s_nssai, k, l);
            }
            ...
            if (IS_SA_MODE(get_softmodem_params())) {
              sprintf(aprefix, "%s.[%d].%s", GNB_CONFIG_STRING_GNB_LIST, k, GNB_CONFIG_STRING_SCTP_CONFIG);
              GET_PARAMS(SCTPParams, GNBSCTPPARAMS_DESC, aprefix);
            ...
            // NETWORK_INTERFACES
            sprintf(aprefix, "%s.[%d].%s", GNB_CONFIG_STRING_GNB_LIST, k, GNB_CONFIG_STRING_NETWORK_INTERFACES_CONFIG);
