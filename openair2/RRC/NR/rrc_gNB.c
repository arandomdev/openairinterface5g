...
static void rrc_gNB_trigger_nsa_release(module_id_t mod_id, rnti_t rnti, int assoc_id)
{
  gNB_RRC_INST *rrc = RC.nrrrc[mod_id];
  rrc_gNB_ue_context_t *ue_context = rrc_gNB_get_ue_context_by_rnti_any_du(rrc, rnti);
  if (!ue_context) {
    LOG_E(NR_RRC, "could not find UE for RNTI %04x\n", rnti);
    return;
  }
  gNB_RRC_UE_t *UE = &ue_context->ue_context;
  if (UE->x2_target_assoc != assoc_id) {
    LOG_E(NR_RRC, "UE RNTI %04x has X2 assoc id %d, but request came from %d\n", rnti, UE->x2_target_assoc, assoc_id);
    return;
  }
  rrc_remove_nsa_user(rrc, ue_context);
}

void rrc_gNB_process_release_request(const module_id_t gnb_mod_idP, x2ap_ENDC_sgnb_release_request_t *m)
{
  rrc_gNB_trigger_nsa_release(gnb_mod_idP, m->rnti, m->assoc_id);
}

void rrc_gNB_process_dc_overall_timeout(const module_id_t gnb_mod_idP, x2ap_ENDC_dc_overall_timeout_t *m)
{
  AssertFatal(false, "not implemented: noone sends this message currently\n");
}
