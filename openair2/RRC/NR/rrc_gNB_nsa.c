...
void rrc_add_nsa_user(gNB_RRC_INST *rrc, x2ap_ENDC_sgnb_addition_req_t *m)
{
  ...
  f1ap_drb_to_be_setup_t drbs = {
      ...
      .rlc_mode = rrc->configuration.um_on_default_drb ? F1AP_RLC_MODE_UM_BIDIR : F1AP_RLC_MODE_AM,
      .up_ul_tnl_length = 1,
  };
...
void rrc_remove_nsa_user(gNB_RRC_INST *rrc, rrc_gNB_ue_context_t *ue_context)
{
  ...
  gtpv1u_delete_all_s1u_tunnel(rrc->module_id, UE->rrc_ue_id);
