...
static bool e1_decode_security_info(security_information_t *out, const E1AP_SecurityInformation_t *in)
{
  ...
  // Encryption Key (M)
  _E1_EQ_CHECK_LONG(in->uPSecuritykey.encryptionKey.size, E1AP_SECURITY_KEY_SIZE);
  ...
  if (in->uPSecuritykey.integrityProtectionKey) {
    ...
    _E1_EQ_CHECK_LONG(ipKey->size, E1AP_SECURITY_KEY_SIZE);
