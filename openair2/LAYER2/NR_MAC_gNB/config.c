...
static void nr_mac_update_sib1_w_SI(gNB_MAC_INST *mac, int num_sibs, int sibs[num_sibs])
{
  NR_COMMON_channels_t *cc = &mac->common_channels[0];
  NR_BCCH_DL_SCH_Message_t *sib1 = cc->sib1;
  sib1 = update_SIB1_NR_SI(sib1, num_sibs, sibs);
  f1ap_setup_req_t *sr = mac->f1_config.setup_req;
  DevAssert(sr->num_cells_available == 1);
  f1ap_served_cell_info_t *info = &sr->cell[0].info;
  prepare_du_configuration_update(mac, info, cc->mib, sib1);
  cc->sib1 = sib1;
...
void nr_mac_configure_other_sib(gNB_MAC_INST *nrmac, int num_cu_sib, const f1ap_sib_msg_t cu_sib[num_cu_sib])
{
  ...
  nr_mac_update_sib1_w_SI(nrmac, num_cu_sib + num_du_sib, config_sibs);
