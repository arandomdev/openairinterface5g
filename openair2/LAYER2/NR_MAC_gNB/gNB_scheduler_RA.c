...
static unsigned int get_slot_RA(NR_ServingCellConfigCommon_t *scc,
                         ...
                         uint32_t preamble_power)
{
  ...
  uint32_t slot_RA = get_slot_RA(scc, rach_ConfigCommon, msgacc, cc, slot);
                                            ...
                                            nfapi_nr_ul_dci_request_t *ul_dci_req)
{
  ...
  if (is_ul_slot(sched_slot, &nr_mac->frame_structure)) {
    ...
    if (!pdcch_pdu_rel15) {
      ...
      nr_configure_pdcch(pdcch_pdu_rel15, coreset, &UE->UE_sched_ctrl.sched_pdcch, false);
                            ...
                            int rbSize)
{
  ...
  if (!pdcch_pdu_rel15) {
    ...
    nr_configure_pdcch(pdcch_pdu_rel15, coreset, &sched_ctrl->sched_pdcch, false);
    nr_mac->pdcch_pdu_idx[CC_id][coresetid] = pdcch_pdu_rel15;
  }
                                  ...
                                  nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  if (is_dl_slot(slotP, &nr_mac->frame_structure)) {
    ...
    if (current_harq_pid < 0) {
      ...
      mac_rlc_status_resp_t srb_status = mac_rlc_status_ind(module_idP, UE->rnti, module_idP, frameP, slotP, ENB_FLAG_YES, MBMS_FLAG_NO, lcid, 0, 0);

      if (srb_status.bytes_in_buffer == 0) {
        lcid = DL_SCH_LCID_DCCH;
        // Check for data on SRB1 (RRCReestablishment, RRCReconfiguration)
        srb_status = mac_rlc_status_ind(module_idP, UE->rnti, module_idP, frameP, slotP, ENB_FLAG_YES, MBMS_FLAG_NO, lcid, 0, 0);
      }
