    ...
    if (!UE->is_synchronized) {
      ...
      *syncMsg = (syncData_t){0};
      ...
      if (UE->UE_scan_carrier == SCAN_BW) {
        ...
        syncMsg->numGscn =
            get_scan_ssb_first_sc(&dl_freq_khz, fp->N_RB_DL, fp->nr_band, fp->numerology_index, 0, 0, 0 /*not used*/, syncMsg->gscnInfo);
        ...
        uint32_t dlFreq = 0;
        syncMsg->numGscn =
            get_scan_ssb_first_sc(&dlFreq, fp->N_RB_DL, fp->nr_band, fp->numerology_index, num_scans, first_scanned_gscn, last_scanned_gscn, syncMsg->gscnInfo);
        num_scans++;
