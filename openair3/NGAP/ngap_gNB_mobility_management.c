...
NGAP_NGAP_PDU_t *encode_ng_handover_required(const ngap_handover_required_t *msg)
{
  ...
  for (int i = 0; i < msg->nb_of_pdusessions; ++i) {
    ...
    *directFwdPathAvl = NGAP_DirectForwardingPathAvailability_direct_path_available; // TODO set False! (?)
  ...
  source2target->rRCContainer.size = msg->source2target->handoverInfo.length;
  source2target->rRCContainer.buf = malloc_or_fail(msg->source2target->handoverInfo.length);
  memcpy(source2target->rRCContainer.buf, msg->source2target->handoverInfo.buffer, source2target->rRCContainer.size);
...
int decode_ng_handover_request(ngap_handover_request_t *out, const NGAP_NGAP_PDU_t *pdu)
{
  ...
  octet_string_to_ngap_pdu(&sourceToTargetTransparentContainer, ie->value.choice.SourceToTarget_TransparentContainer);
  ...
  asn_dec_rval_t dec_rval = aper_decode_complete(NULL,
                                                 ...
                                                 (void **)&source2target,
                                                 sourceToTargetTransparentContainer.buffer,
                                                 sourceToTargetTransparentContainer.length);
  ...
  out->ue_ho_prep_info.buf = calloc_or_fail(source2target->rRCContainer.size, sizeof(uint8_t));
  memcpy(out->ue_ho_prep_info.buf, source2target->rRCContainer.buf, source2target->rRCContainer.size);
  out->ue_ho_prep_info.len = source2target->rRCContainer.size;

  // Decode HO Preparation Information
  ...
  for (int pduSesIdx = 0; pduSesIdx < out->nb_of_pdusessions; ++pduSesIdx) {
    ...
    // PDU Session ID (M)
    out->pduSessionResourceSetupList[pduSesIdx].pdusession_id = item_p->pDUSessionID;
    out->pduSessionResourceSetupList[pduSesIdx].pdu_session_type = PDUSessionType_ipv4;
    // S-NSSAI (M)
    out->pduSessionResourceSetupList[pduSesIdx].nssai = decode_ngap_nssai(&item_p->s_NSSAI);
    // Handover Request Transfer (M)
    octet_string_to_ngap_pdu(&out->pduSessionResourceSetupList[pduSesIdx].pdusessionTransfer, item_p->handoverRequestTransfer);
  }
  return 0;
}
...
NGAP_NGAP_PDU_t *encode_ng_handover_request_ack(ngap_handover_request_ack_t *msg)
{
  ...
  // Target to Source Transparent Container (M)
  {
    ...
    // RRC Container (M)
    {
      ...
      t2s->rRCContainer.size = msg->target2source.length;
      t2s->rRCContainer.buf = malloc_or_fail(msg->target2source.length);
      memcpy(t2s->rRCContainer.buf, msg->target2source.buffer, msg->target2source.length);
      uint8_t *buf = NULL;
      int encoded =
          aper_encode_to_new_buffer(&asn_DEF_NGAP_TargetNGRANNode_ToSourceNGRANNode_TransparentContainer, NULL, t2s, (void **)&buf);
      OCTET_STRING_fromBuf(&ie->value.choice.TargetToSource_TransparentContainer, (const char *)buf, encoded);
    }
  }
