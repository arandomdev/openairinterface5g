...
static void ldpc8blocks(void *p)
{
  ...
  for (int r = macro_segment; r < macro_segment_end; r++) {
    ...
    if(args->toutput != NULL) start_meas(args->toutput);
    ...
    if ((offset_output_bit & 7) == 0) {
      ...
      uint64_t *output_64 = (uint64_t *)&nrLDPC_TB_encoding_parameters->output[(offset_output_bit >> 3) & 7];
    ...
    if(args->toutput != NULL) stop_meas(args->toutput);
